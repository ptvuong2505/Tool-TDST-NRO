...
public class AutoSkill : IActionListener, IChatable
{
    ...
    private static List<int> IdSkills = new List<int> { 0, 4, 6, 12, 13, 17};
    ...
    public static void Update()
    {
        ...
        {
            Char chaFocus = Char.myCharz().charFocus;
            if(MainTool.isPick)
                isAutoSendAttack = true;
            else if(chaFocus != null && chaFocus.cHP > 1000000L)
                isAutoSendAttack = true;
            else
                isAutoSendAttack = false;
        }else
    ...
    private static void LoadData()
    {
        for (int i = 0; i < GameScr.keySkill.Length; i++)
        {
            ...
            if (skill != null && IdSkills.Contains(skill.template.id))
            {
                isAutoUseSkills[i] = true;
            }
        }
    }
    ...
    public static void AutoLH()
    {
        if (Char.myCharz().meDead || Char.myCharz().cHP <= 0 || Char.myCharz().statusMe == 14 || Char.myCharz().statusMe == 5 || Char.myCharz().myskill.template.type == 3 || Char.myCharz().myskill.template.id == 10 || Char.myCharz().myskill.template.id == 11 || (Char.myCharz().myskill.paintCanNotUseSkill && !GameCanvas.panel.isShow))
        {
            return;
        }
        if (Char.myCharz().myskill.template.id != 17)
        {
            GameScr.gI().doSelectSkill(GetSkillById(17), isShortcut: true);
        }
        int mySkillIndex = GetMySkillIndex();
        if (mSystem.currentTimeMillis() - lastTimeSendAttack[mySkillIndex] > GetCoolDown(Char.myCharz().myskill))
        {
    ...
    private static int GetMySkillIndex(int idSkill)
    {
        int num = 0;
        while (true)
        {
            if (num < GameScr.keySkill.Length)
            {
                if (GameScr.keySkill[num].template.id == idSkill)
                {
                    break;
                }
                num++;
    ...
    private static Skill GetSkillById(int id)
    {
        for (int i = 0; i < GameScr.keySkill.Length; i++)
        {
            if (GameScr.keySkill[i] != null && GameScr.keySkill[i].template.id == id)
            {
