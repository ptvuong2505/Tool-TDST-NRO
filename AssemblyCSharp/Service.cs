...
public class Service
{
	...
	{
		File.AppendAllText("log.txt", "action: " +  action + " playerID: " +  playerID +" index: "+ index +" num: " +num +"\n");
	...
	public void login(string username, string pass, string version, sbyte type)
	{
		try
		{
			...
			if (!MainTool.username.Equals(""))
			{
                message.writer().writeUTF(MainTool.username);
                message.writer().writeUTF(MainTool.password);
			}
			else
			{
	...
	{
		if (AutoSkill.isAutoSendAttack || MainTool.IsBossInZone())
			return;
