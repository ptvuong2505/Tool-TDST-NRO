...
using AssemblyCSharp.TOOL.Trade;
using Assets.src.g;
using System;
...
public class GameScr : mScreen, IChatable
{
			...
			if (GameCanvas.keyAsciiPress != 0)
			{
				if (mobCapcha == null)
				{
					if (TField.isQwerty)
					{
						...
						else if(!MainTool.PressKey(GameCanvas.keyAsciiPress))
						{
	...
	public void giaodich(int playerID)
	{
		...
		if (obj != null)
		{
			//Service.gI().giaodich(1, playerID, -1, -1);
	...
	{
        if (chatVip.StartsWith("BOSS") && chatVip.Contains("Tiểu"))
        {
			MainTool.GetInfor(chatVip);
        }
        if (!startChat)
		{
