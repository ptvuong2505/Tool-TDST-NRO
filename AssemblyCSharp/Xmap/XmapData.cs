...
namespace AssemblyCSharp.Xmap
{
    public class XmapData
    {
        ...
        private void LoadLinkMapBase()
        {
            ...
            LoadLinkMapSieuThi();
            LoadLinkMapToCold();
            //auto vào nrd 7s (có ở đây thì không cần trong txt)
            //MyLinkMaps[24].Add(new MapNext(6, TypeMapNext.NpcPanel, new int[] { 29, 0, -1 }));
            //MyLinkMaps[25].Add(new MapNext(6, TypeMapNext.NpcPanel, new int[] { 29, 0, -1 }));
            //MyLinkMaps[26].Add(new MapNext(6, TypeMapNext.NpcPanel, new int[] { 29, 0, -1 }));
        }
